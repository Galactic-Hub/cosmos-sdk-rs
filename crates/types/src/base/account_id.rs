    ...
    pub fn new(prefix: &str, bytes: &[u8]) -> Result<Self, Error> {
        ...
        if !prefix.chars().all(|c| matches!(c, 'a'..='z' | '0'..='9')) {
            return Err(Error::InvalidAccountId(format!(
                "expected prefix to be lowercase alphanumeric characters only, accountID({:?})",
                id
            )));
        }
        ...
        if matches!(bytes.len(), 1..=Self::MAX_LENGTH) {
            ...
            Err(Error::InvalidAccountId(format!(
                "account ID should be at most {} bytes long, but was {} bytes long, AccountId({:?})",
                Self::MAX_LENGTH,
                bytes.len(), id
            )))
        }
...
impl FromStr for AccountId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let (hrp, bytes) = bech32::decode(s)
            .map_err(|e| Error::Custom(format!("invalid bech32: '{}', Error({e:?})", s)))?;
        Self::new(&hrp, &bytes)
    }
}
    ...
    fn try_from(id: &AccountId) -> Result<tendermint::account::Id, Self::Error> {
        ...
        match bytes.try_into() {
            ...
            _ => Err(Error::Custom(format!(
                "invalid length for account ID: {}, AccountId({})",
                len,
                id.bech32.clone()
            ))),
        }
